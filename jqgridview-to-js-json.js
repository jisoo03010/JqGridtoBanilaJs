    ...
    let gridView = function() {
        return {
            build: function(div, cfg) {
                ...
                if (cfg.modifiedRows != undefined) {
                    if (cfg.modifiedRows.length > 0) {
                        ...
                        if (gridInsertPanelBody) {
                          gridInsertPanelBody.innerHTML = `<b>${cfg.messages.saveChanges.text}</b>`;
                        }
                ...
                function buildHeader(json, config) {
                    let tr = document.createElement('tr');
                    let thead = document.querySelector('#' + config.tableId + ' thead');
                    config.headers = [];
                
                    let headerElements = json.response.gridViewData.header.element;
                    headerElements.forEach((headerElement, i) => {
                        let prevWidth = config.prevWidths ? config.prevWidths[i] : null;
                        config.headers[i] = {
                            title: headerElement.attributes.title,
                            name: headerElement.attributes.name,
                            initWidth:  headerElement.attributes.width,
                            resizable: parseInt(headerElement.attributes.resizable || 0, 10),
                            sortable: parseInt(headerElement.attributes.sortable || 1, 10),
                            isId: parseInt(headerElement.attributes.isId || 0, 10),
                            contentAlign: headerElement.attributes.contentAlign || 'left',
                            editTemplateId: headerElement.attributes.editTemplateId,
                            itemTemplateId: headerElement.attributes.itemTemplateId,
                            order: (headerElement.attributes.name === json.response.parameters.orderBy.textContent)
                                ? json.response.parameters.order.textContent
                                : 'desc'
                        };
                        ...
                        div.innerHTML = headerElement.attributes.title || "&nbsp;";
                        ...
                        if (config.headers[i].sortable === 1) {
                            ...
                            sortImg.src = `${config.imagesUrl}`+ 'asc.gif'; 
                            ...
                            sortDiv.addEventListener('click', () => {
                                ...
                                sortImg.src =  `${config.imagesUrl}`+  newOrder + '.gif';
                            });
                        }
                ...
                function buildBody(json, config) {
                    config.orderBy = json.response.parameters.orderBy || '';
                    config.order = json.response.parameters.order || '';
                    config.currentPage = parseInt(json.response.parameters.currentPage.text) || 1;
                    config.totalPages = parseInt(json.response.parameters.totalPages.text) || 1;
                    config.totalItems = parseInt(json.response.parameters.totalItems.text) || 0;
                    config.pageSize = config.pageSize || 10;
                
                    const currentFilterNode = json.response.parameters.currentFilter;
                    config.currentFilter = {
                        name: currentFilterNode?.json.response.gridViewData.header.element.attributes.name || '',
                    ...
                    const rows = json.response.gridViewData.rows.row;
                    config.rows = [];
                    ...
                    for (let i = startIndex; i < endIndex; i++) {
                        ...
                        row.cell.forEach((cell) => {
                            ...
                            if (name) config.rows[i][name] = cell.text; 
                        });
                        ...
                        for (let j = 0; j < config.headers.length; j++) {
                            ...
                            if (header.editTemplateId) {
                                ...
                                div.addEventListener('dblclick', buildEditTemplate); // 기존 함수 사용
                            }
                    
                            // Item 템플릿 처리
                            if (header.itemTemplateId) {
                                const templateId = header.itemTemplateId;
                                const templateNode = jsonData.response.gridViewData.itemTemplates.find(
                                    (template) => template.attributes.id === templateId
                                );
                                let template = templateNode?.content?.templatebody || '';
                    
                                templateNode?.content?.entry.forEach((param) => {
                                    const key = param.attributes.key;
                                    const value = config.rows[i][param.attributes.value] || '';
                                ...
                                // 기본 셀 데이터 설정
                                const cellData = config.rows[i][header.name] || '&nbsp;';
                        ...
                        tbody.appendChild(tr); // 최종적으로 tbody에 추가
                    }
                    
                }
                

                function buildGridTools(json, config) {
                    console.log('buildGridTools !!');
                    
                    config.messages = {};
                    config.messages['saveChanges'] = json.response.messages.saveChanges.text|| 'Save changes?';
                    config.messages['deleteLine'] = json.response.messages.deleteLine.text || 'Delete line?';
                
                    config.filters = [];
                
                    const filters = json.response.filterLayer.filter
                    filters.forEach((filter, i) => {
                        config.filters[i] = {
                            id: filter.id,
                            value: filter.value,
                            title: filter.title || filter.value,
                            isQuoted: filter.isQuoted || 0
                        };
                        const operators = filter.operators || [];
                    
                        config.filters[i].operators = operators.map((operator) => ({
                            title: operator.title || operator.value,
                            value: operator.value
                        }));
                    });
                
                    config.panelEntries = [];
                    const panelEntries = json.response.panelEntries.entry;
                    panelEntries.forEach((entry) => {
                        config.panelEntries.push({
                            type: entry.attributes.type || 'custom',
                            value: entry.textContent
                        });
                    });
                
                    config.translations = {};
                    const translations = json.response.translations.add;
                    translations.forEach((translation) => {
                        const key = translation.attributes.key.toLowerCase();
                        const value = translation.attributes.value;
                        config.translations[key] = value;
                    });
                
                    config.editTemplates = {};
                    const editTemplates = json.response.editTemplates.template;
                    editTemplates.forEach((template) => {
                        const tid = template.attributes.id;
                        ...
                        switch (config.editTemplates[tid].type) {
                            ...
                            case 'checkBox':
                                ...
                                const checkBoxEntries = template.content.entry;
                            ...
                            case 'dropDownList':
                                config.editTemplates[tid].entries = [];
                                config.editTemplates[tid].valueColumn = template.attributes.valueColumn;
                                const dropDownEntries = template.content.entry;
                    ...
                    const insertTemplate = json.response.insertTemplates.template;
                    if (insertTemplate) {
                        config.insertTemplate = insertTemplate;
                    }
                }

                function checkResponseState(json, config) {
                    ...
                    const gridViewData = json.response.gridViewData;
                    if (!gridViewData || json.response.gridViewData.rows.row.length === 0) {
                        if (messageDiv) {
                            messageDiv.innerHTML = '<b style="color:red;">Error: json data file is not well-formed !</b>';
                        }
                    ...
                    else if (json.response.messages.errorMessage?.textContent.trim().length > 0) {
                        const errorMessage = json.response.messages.errorMessage?.textContent.trim();
                        if (messageDiv) {
                    ...
                    else if (json.response.parameters.totalItems.innerHTML === '0') {
                        const noResultsMessage = json.messages.noResultsMessage?.textContent;
                        if (messageDiv) {
            ...
            init: function(config) {
                ...
                function createInsertLayer() {
                    console.log("createInsertLayer 실행!", config.insertTemplate);
                ...
                function showFilterLayer(e) {
                    console.log('config.filterLayer : ', config.filterLayer);
                    config.filterLayer.style.display = 'block';
                }

                function showInsertLayer(e) {
                    console.log('config.showInsertLayer : ', config.insertLayer);
